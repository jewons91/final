...
# 데이터 로드
df = pd.read_csv('C:\\big18\\final\\with\\project\\test\\data\\000660전체데이터Vector.csv')
# df = df.iloc[:,2:]
...
# 데이터셋 분리 (순서를 유지하면서 나누기)
split_ratio = 0.8
split_index = int(num_rows * split_ratio)

train_indices = range(split_index)
val_indices = range(split_index, num_rows)
...
class GCN(torch.nn.Module):
    def __init__(self, num_node_features, num_classes):
        ...
        # 1D Convolution Layer 추가
        self.conv1d = Conv1d(in_channels=1, out_channels=64, kernel_size=3, padding=1)
    ...
    def forward(self, data):
        ...
        # 1D Conv Layer를 사용하기 위해 차원 변환
        x = x.unsqueeze(1)  # (N, C, L) 형태로 변환
        x = self.conv1d(x)  # 1D Convolution
        x = x.squeeze(1)  # 다시 차원 축소
        # x = self.dropout(x)
    ...
    if epoch % 10 == 0:
        ...
        if epoch > 500:
            if val_loss < best_loss:
                best_loss = val_loss
                torch.save(model.state_dict(), 'C:\\big18\\final\\with\\project\\test\\gcn변동률.pth')
                early_stopping_counter = 0
...
# 학습 완료 후 최종 평가
model.load_state_dict(torch.load('C:\\big18\\final\\with\\project\\test\\gcn변동률.pth'))
model.eval()
with torch.no_grad():
