...
for stock_name in stock_names:
    ...
    # 데이터 로드
    df = pd.read_csv(f'C:\\big18\\final\with\project\data\\validation\preprocessed_{stock_name}.csv')
    print(df)
    # df = df.iloc[:,2:]
    ...
    # 학습 완료 후 최종 평가
    model.load_state_dict(torch.load('C:\\big18\\final\\with\\project\\model_info\\gcn_data_129_acu86.pth'))
    model.eval()
    with torch.no_grad():
    ...
    result_df['Real'] = val_true
    result_df['Pred'] = val_pred
    result_df['Correct'] = np.where(result_df['Real'] == result_df['Pred'], '일치', '불일치')
    
    result_df.to_csv(f'C:\\big18\\final\with\project\data\\validation\\result_{stock_name}.csv', index=False)
