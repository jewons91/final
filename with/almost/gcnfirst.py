...
# 데이터 로드
df = pd.read_csv('C:/big18/dl-dev/dl-dev/project/변동률과 체결강도/학습전체데이터/전처리데이터/022100전체데이터Vector.csv')
# df = df.iloc[:,2:]
...
model.load_state_dict(torch.load('C:/big18/dl-dev/dl-dev/project/변동률과 체결강도/gcn전이하_삼_에_홀_비_현_알_한_퓨.pth'))
...
for epoch in range(100000):
    ...
    if epoch % 10 == 0:
        ...
        if epoch > 500:
            if val_acc > best_acc:
                best_acc = val_acc
                torch.save(model.state_dict(), 'C:/big18/dl-dev/dl-dev/project/변동률과 체결강도/gcn전이하_삼_에_홀_비_현_알_한_퓨_D.pth')
                early_stopping_counter = 0
...
# 학습 완료 후 최종 평가
model.load_state_dict(torch.load('C:/big18/dl-dev/dl-dev/project/변동률과 체결강도/gcn전이하_삼_에_홀_비_현_알_한_퓨_D.pth'))
model.eval()
with torch.no_grad():
...
result_acc_map[count_number] = (conf_matrix, class_report, final_acc)


# # 파일 옮기기
# source_dir = 'C:/big18/dl-dev/dl-dev/project/변동률과 체결강도/학습전체데이터/전처리데이터'
# destination_dir = 'C:/big18/dl-dev/dl-dev/project/변동률과 체결강도/학습전체데이터/학습완료데이터'
