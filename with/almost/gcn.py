...
for code in codes:
    ...
    # val_indices = data_indices[split_index:]
    ...
    # model.load_state_dict(torch.load(f'C:\\big18\\final\\with\\almost\\gcn_model\\gcn_반도체{count_number}.pth'))
    # count_number += 1
    ...
    for epoch in range(100000):
        ...
        if epoch % 10 == 0:
            ...
            if epoch > 500:
                if val_acc > best_acc:
                    best_acc = val_acc
                    torch.save(model.state_dict(), f'C:\\big18\\final\\with\\almost\\gcn_model\\gcn_2단방향{count_number}.pth')
                    early_stopping_counter = 0
    ...
    # 학습 완료 후 최종 평가
    model.load_state_dict(torch.load(f'C:\\big18\\final\\with\\almost\\gcn_model\\gcn_2단방향{count_number}.pth'))
    model.eval()
    with torch.no_grad():
    ...
    source_dir = 'C:\\big18\\final\\with\\almost\\data'
    destination_dir = 'C:\\big18\\final\\with\\almost\\finished_data'
    file_name = f'{code}전체데이터Vector.csv'
